...
/**
 * The client side of the TMail application. Allows sending and getting messages
...
public class ClientMsgApplication {
	...
	/**
	 * Logs the client to the server. Any incoming messages from the server will
	 * be routed to the provided consumer. If the client missed any messages
	 * while he was offline, all of these will be first routed to client in the
	...
	/**
	 * Logs the client out, cleaning any resources the client may be using. A
	 * logged out client cannot accept any messages. A client can login (using
	 * {@link ClientMsgApplication#login(Consumer, Function, BiConsumer)} after
	 * logging out.
	 */
	public void logout() {
		throw new UnsupportedOperationException("Not implemented");
	...
	/**
	 * Requests the friendship of another user. Friends can see each other
	 * online using {@link ClientMsgApplication#isOnline(String)}. Friend
	 * requests are handled similarly to messages. An incoming friend request is
	 * consumed by the friendRequestsConsumer. An incoming friend request
	 * <i>reply</i> is consumed by the friendRequestRepliesConsumer.
	 * 
	 * @param who
	...
	/**
	 * Checks if another user is online; the client can only ask if friends are
	 * online
	 * 
	 * @param who
	 *            The person to check if he is online
	 * @return A wrapped <code>true</code> if the user is a friend and is
	 *         offline; a wrapped <code>false</code> if the user is a friend and
	 *         is offline; an empty {@link Optional} if the user isn't a friend
	 *         of the client
	 */
	 ...
	 * This is mainly used to clean resource use in test cleanup code. You can
	 * assume that a stopped client won't be restarted using
